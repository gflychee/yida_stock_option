...
private:
	...
	/// 当前所有的挂单，LongOrderSysID->YDOrder
	std::map<long long,YDOrder> m_pendingOrders;
	//当系统报单号没有超过int的表示范围时，也可以使用OrderSysID->YDOrder
	...
	void updateOrder(const YDOrder *pOrder)
	{
		...
		YDOrder *pOldOrder;
		std::map<long long,YDOrder>::iterator it=m_pendingOrders.find(pOrder->LongOrderSysID);
		...
		else
		{
			...
			{
				m_pendingOrders[pOrder->LongOrderSysID]=*pOrder;
			}
		}
	}
...
public:
	...
	virtual void notifyOrder(const YDOrder *pOrder, const YDInstrument *pInstrument, const YDAccount *pAccount)
	{
		...
		if (pOrder->OrderStatus==YD_OS_Queuing)
		{
			...
			cancelOrder.LongOrderSysID=pOrder->LongOrderSysID;
			//当系统报单号没有超过int的表示范围时，也可以使用下面的语句
